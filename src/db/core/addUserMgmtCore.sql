...
--Define a temporary function to perform the upgrade
CREATE OR REPLACE FUNCTION pg_temp.upgradeConnectionActivity()
RETURNS VOID AS
$$
BEGIN
...
$$
BEGIN
   --If ClassDB.ConnectionActivity is present and contains data, upgrade it rather
   -- than recreate it and lose existing data
   --Use isColumnDefined to check if ClassDB.ConnectionActivity exists - column
   -- username is present in all versions
   IF  ClassDB.isColumnDefined('ClassDB', 'ConnectionActivity', 'UserName') THEN
      IF EXISTS (SELECT * FROM ClassDB.ConnectionActivity) THEN
         PERFORM pg_temp.upgradeConnectionActivity();
      END IF;
   ELSE
